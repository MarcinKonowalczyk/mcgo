...
/* Init the subsystem. The argument is the limit on no. of bytes to allocate, 0
 * if no limit */
void
...
unsigned int
slabs_clsid(unsigned int size);

/* Allocate object of given size class identified by id. 0 on error */
void *
slabs_alloc(unsigned int id);

/* Free previously allocated object */
void
slabs_free(void *ptr, unsigned int id);
...
int
item_link(item *it); /* may fail if transgresses limits */
void
item_unlink(item *it);
void
item_remove(item *it);
...
void
item_update(item *it); /* update LRU time to current and reposition */
int
